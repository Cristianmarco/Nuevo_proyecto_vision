  ...
  try {
    ...
    res.json({
      ...
      dota: {
        ...
        vencidos: Number(vencidosDota.rows[0].count),
      },
      totales: {
        vigentes: Number(vigentesTot.rows[0].count),
        garantias: Number(garantiasTot.rows[0].count),
        vencidos: Number(vencidosTot.rows[0].count),
      }
    });
  } catch (err) {
    next(err);
